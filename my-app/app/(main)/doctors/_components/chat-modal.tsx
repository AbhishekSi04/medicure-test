...
import { User, Send, Loader2, MessageSquare, X, Stethoscope } from 'lucide-react';
...
export default function ChatModal({ isOpen, onClose, doctor, patient, chatRoomId }: ChatModalProps) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await sendMessage({ chatRoomId, senderId: patient.id, messageType: 'TEXT', content: inputMessage.trim() });
      setMessages(prev => [...prev, msg]);
      setInputMessage('');
      ...
      <DialogContent className="sm:max-w-[560px] h-[85vh] flex flex-col p-0 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl bg-white dark:bg-slate-950 gap-0">
        ...
        <div className="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-blue-600 to-indigo-700 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-white font-bold text-sm truncate">Dr. {doctor?.name || 'Doctor'}</p>
            <p className="text-blue-200 text-xs">{doctor?.specialty || 'Healthcare Professional'}</p>
          </div>
          ...
          {isLoadingHistory ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
              ...
              <p className="text-slate-400 dark:text-slate-500 text-xs max-w-[200px]">
                Say hello to Dr. {doctor?.name || 'your doctor'}!
              </p>
            </div>
          ) : (
            (() => {
              ...
              return messages.map((message, index) => {
                ...
                return (
                  <div key={`msg-${index}`}>
                    ...
                    {/* Message bubble */}
                    <div className={`flex items-end gap-2 mb-1.5 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                      {/* Avatar */}
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mb-0.5 ${isUser ? 'bg-gradient-to-br from-blue-500 to-indigo-600' : 'bg-gradient-to-br from-slate-500 to-slate-700'}`}>
                        {isUser
                      ...
                      <div className="max-w-[72%] group">
                        {/* Sender name */}
                        <p className={`text-[10px] font-semibold mb-1 px-1 ${isUser ? 'text-right text-blue-600 dark:text-blue-400' : 'text-left text-slate-500 dark:text-slate-400'}`}>
                          {isUser ? 'You' : `Dr. ${doctor?.name || 'Doctor'}`}
                        </p>
                        ...
                        >
                          ...
                          <p className={`text-[10px] mt-1.5 ${isUser ? 'text-blue-100/80 text-right' : 'text-slate-400 dark:text-slate-500 text-right'}`}>
